...
const InfoBanner: React.FunctionComponent<Props> = ({ movieData, isLiked, isSaved, rating }) => {
  return (
    <div
      className={`w-full h-fit bg-cover bg-center bg-no-repeat bg-[url(${imageUrlBase}${movieData.poster_path})]`}
    >
      <div className="w-full h-full bg-linear-to-r from-black/100 from-10% md:from-5% to-black/80 to-40% md:to-25%">
        <div className="flex flex-row columns-2 p-7 pl-8 h-full w-full ">
          <div id="poster" className="relative min-w-[200px] mr-5 aspect-2/3">
            ...
            <img
              src={`${imageUrlBase}${movieData.backdrop_path}`}
              alt={movieData.title}
